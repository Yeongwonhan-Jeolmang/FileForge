...
def extract_strings(file_path: str, min_length: int = 4, encoding: str = 'ascii') -> List[Tuple[int, str]]:
    ...
    try:
        ...
        if encoding == 'ascii':
            ...
            printable = re.compile(b'[\x20-\x7E]{' + str(min_length).encode() + b',}')
        elif encoding == 'utf-8':
            # UTF-8 printable characters
            printable = re.compile(b'[\x20-\x7E\xC2-\xF4][\x80-\xBF]*{' + str(max(0, min_length-1)).encode() + b',}')
        elif encoding == 'utf-16':
            # UTF-16 (little endian) - this is more complex
            # For simplicity, we'll use a basic approach
            printable = re.compile(b'(?:[\x20-\x7E]\x00){' + str(min_length).encode() + b',}')
        elif encoding == 'latin-1':
            # Latin-1 printable characters
            printable = re.compile(b'[\x20-\xFF]{' + str(min_length).encode() + b',}')
        else:
            # Default to ASCII
            printable = re.compile(b'[\x20-\x7E]{' + str(min_length).encode() + b',}')
